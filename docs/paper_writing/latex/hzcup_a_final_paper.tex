...
正式支撑材料建议见表\ref{tab:support-files}。
...
类型 & 路径 & 说明 \\
\midrule
论文母稿 & \texttt{docs/paper\_writing/hzcup\_a\_final\_paper\_mother\_draft.md} & 当前 Markdown 母稿 \\
全题收官总结 & \texttt{docs/paper\_writing/project\_closeout\_full\_summary.md} & 正式结果和写作边界 \\
第一问正式输出 & \texttt{outputs/problem1/} & 静态基准调度结果 \\
第二问正式输出 & \texttt{outputs/problem2/} & 绿色限行推荐方案 \\
第三问正式输出 & \texttt{outputs/problem3/} & 代表性动态情景结果 \\
模型检验输出 & \texttt{outputs/model\_validation/} & 可行性、收敛、敏感性图表 \\
图表生成数据包 & \texttt{outputs/gpt\_pro\_visual\_pack/} & 图表 CSV、数值自检记录和生成说明 \\
代码目录 & \texttt{green\_logistics/} & 数据处理、成本、调度和求解模块 \\
运行入口 & \texttt{problems/problem1.py}，\texttt{problem2.py}，\texttt{problem3.py} & 三问复现入口 \\
测试目录 & \texttt{tests/} & 单元与集成测试 \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{附录 C：长表与图表生成说明}
正文图像已嵌入 LaTeX 源文件，图像原始数据、数值自检记录和生成数据包随支撑材料一并提交。长路线表、逐停靠明细和动态诊断文件不放入正文。
\end{document}
